...
describe( "LinearScale", () => {
	...
	describe( "Color Scale", () => {
		it( "should scale two rgb colors", () => {
			let c = lin.domain( [0, 1] ).range( ["red", "blue"] ).get( 0.5 );
			expect( c ).to.equal( "rgb(128, 0, 128)" );
		} );

		it( "should scale two hsl colors", () => {
			let c = lin.domain( [0, 1] ).range( ["hsl(120,50%,10%)", "hsl(360,100%,50%)"] ).get( 0.5 );
			expect( c ).to.equal( "rgb(134, 19, 6)" );
		} );

		it( "should scale rgb color to a hsl color", () => {
			let c = lin.domain( [0, 1] ).range( ["blue", "hsl(360,100%,50%)"] ).get( 0.5 );
			expect( c ).to.equal( "rgb(128, 0, 128)" );
		} );

		it( "should scale hsl color to a rgb color", () => {
			let c = lin.domain( [0, 1] ).range( ["hsl(360,100%,50%)", "blue"] ).get( 0.5 );
			expect( c ).to.equal( "rgb(128, 0, 128)" );
		} );
