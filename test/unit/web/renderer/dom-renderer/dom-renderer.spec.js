...
describe('dom renderer', () => {
  ...
  describe('size', () => {
    it('should return current size if no parameters are given', () => {
      rend.appendTo(element('div'));
      rend.size({ x: 50, y: 100, width: 200, height: 400 });
      expect(rend.size()).to.deep.equal({ x: 50, y: 100, width: 200, height: 400, scaleRatio: { x: 1, y: 1 } });
    });

    it('should ignore NaN values and fallback to default size value', () => {
      rend.size({ x: undefined, y: undefined, width: undefined, height: undefined, scaleRatio: { x: undefined, y: undefined } });
      expect(rend.size()).to.deep.equal({ x: 0, y: 0, width: 0, height: 0, scaleRatio: { x: 1, y: 1 } });
    });
  });
});
